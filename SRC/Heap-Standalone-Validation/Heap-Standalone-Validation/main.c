...
#ifdef FREE
if(VALLOC_CHECK)
    free(v_allocs[HEAP_CHUNK]);
else 
    free(allocations[HEAP_CHUNK]);
#endif 

#ifdef HEAPVALIDATE_CHUNK
    if (HeapValidate(defaultHeap, HEAP_NO_SERIALIZE, v_allocs[HEAP_CHUNK])) {
        printf("\nHeap Function Pointer Chunk %d is Valid\n", HEAP_CHUNK);
    }
    else {
        printf("\nHeap Function Pointer Chunk %d is Invalid\n", HEAP_CHUNK);
    }
#endif

#ifdef HEAPVALIDATE_ALL
    if (arg = HeapValidate(defaultHeap, HEAP_NO_SERIALIZE, NULL)) {
        printf("\nHeap is Valid\n");
    }
    else {
        printf("\nHeap is Invalid\n");
